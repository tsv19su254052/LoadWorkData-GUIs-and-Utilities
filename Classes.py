...
class Server:
    ...
    Обмен данными между программами до SQL-ных баз данных:
     ...
     - файл с тэгами и иерархией типа *.xml (xml.dom, xml.sax) или *.html,
        ...
        XQuery & XPath (выведен из SQL и OQL) и XSLT (выведен из SGML с его языком таблиц стилей DSSSL) берут на вход файл XML, обрабатывают его по формуле и выводят файл XML, XHTML, HTML
    ...
    Главная трудоемкость математики - это скалярное <-> векторное преобразование - решена в Python и в R, но не в SQL-ных БД.
    ...
    Поэтому происходит потеря данных, их точности и разнообразия.
    ...
    Как вариант можно сохранить его файлом, но файл - не СУБД (нет способа множественного доступа на межпрограммном уровне с транзакциями и по нескольким учеткам), поэтому центром вычислительной активности в прикладных проектах является Microsoft SQL Server
    ...
    # todo Указать уровень изоляции транзакции - СДЕЛАЛ
    """
    выборка и все остальное - READ COMMITTED (Чтение зафиксированных данных - в настройках по умолчанию) инструкции не могут считывать данные, которые были изменены и еще не зафиксированы другими транзакциями Другие транзакции могут изменять и вставлять данные, читаемые текущей транзакцией
    обновление - REPEATABLE READ (Повторяемость чтения) - Дополнительно: Другие транзакции могут только вставлять данные, читаемые текущей транзакцией
    вставка - SERIALIZABLE (Упорядочиваемость) - Дополнительно: Другие транзакции не могут изменять и вставлять данные в диапазон, читаемые текущей транзакцией
