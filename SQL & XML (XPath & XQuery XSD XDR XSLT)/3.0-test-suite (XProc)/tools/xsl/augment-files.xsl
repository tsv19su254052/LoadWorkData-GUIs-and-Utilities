  ...
  <xsl:template match="c:file">
    <xsl:copy>
      ...
      <xsl:if test="preceding-sibling::c:file[1]">
        <xsl:attribute name="prev" select="substring-before(preceding-sibling::c:file[1]/@name, '.xml')"/>
      </xsl:if>
      <xsl:if test="following-sibling::c:file[1]">
        <xsl:attribute name="next" select="substring-before(following-sibling::c:file[1]/@name, '.xml')"/>
      </xsl:if>
      <xsl:apply-templates select="node()"/>
    </xsl:copy>
  </xsl:template>
