...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:param name="publication.root.uri"
           select="if (/processing-instruction(publication-root)) then xs:string(processing-instruction(publication-root)) else 'http://www.w3.org/TR/'"/>
    ...
    <xsl:variable name="pubdate" select="(xs:date(db:specification/db:info/db:pubdate), current-date())[1]"/>
    <xsl:variable name="dtz" select="adjust-dateTime-to-timezone(current-dateTime(), xs:dayTimeDuration('PT0H'))"/>
