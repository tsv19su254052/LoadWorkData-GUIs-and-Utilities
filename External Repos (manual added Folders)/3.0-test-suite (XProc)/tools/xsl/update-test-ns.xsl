...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="t:test/@code|s:ns/@uri" xmlns:s="http://purl.oclc.org/dsdl/schematron">
    <xsl:copy/>
  </xsl:template>
  ...
  <xsl:template match="attribute()|text()|comment()|processing-instruction()" mode="strip-html">
    <xsl:copy/>
  </xsl:template>
