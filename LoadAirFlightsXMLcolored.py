...
import Classes
# todo  - Сделать пользовательскую наработку (не библиотеку и не пакет) отдельным репозиторием -> Использовал существующую, как пример
#       - Импортировать ее внутри репозитория для повторного применения синхронно (mutualy connected) или асинхронно (independent) -> Импортировал асинхронно, обновление только вручную на командах git
#       - В репозитории на github-е у импортируемой наработки значек "Used by ..." пока не наблюдается
#       - Результат импорта -> на github-е - синяя ссылка, по которой никуда не перейдешь, внутри pyCharm - локальный дубликат репозиториев в локальную ветку
# fixme Есть проблемка при импорте пользовательских библиотек из внешних файлов по другим путям из других папок ...
# todo pyCharm как графическая оболочка пока не поддерживают 'git submodules' [@Aleks10](https://qna.habr.com/q/196071) и связанные с ним команды
