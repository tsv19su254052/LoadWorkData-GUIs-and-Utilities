...
# todo  - Сделать пользовательскую библиотеку отдельным репозиторием (как альтернативу виртуальному окружению) -> Использовал существующую как пример
#       - Импортировать ее внутри репозитория для повторного применения синхронно (mutualy connected) или асинхронно (independent) -> Импортировал, но как, пока не понятно
#       - Посмотреть, чтобы в репозитории на github-е появился значек, что она применяется -> Пока не наблюдается
#       - В импортируемом репозитории на github-е папка с подмодулем должна быть написана синим текстом -> Синяя ссылка, по которой никуда не перейдешь
#       - Будет ли импорт в чистом виде или будет дублировнание места на github-е и на pyCharm-е? -> на github-е - ссылка, внутри pyCharm - локальный дубликат в локальную ветку
# todo Научиться делать это по возможности только внутри pyCharm-а -> pyCharm пока не поддерживают 'git submodules' [@Aleks10](https://qna.habr.com/q/196071)
