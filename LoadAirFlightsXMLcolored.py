...
import Classes
# fixme Есть проблемка при импорте пользовательских библиотек из внешних файлов по другим путям из других папок ... -> Выхожу из положения по-разному, иногда не срабатывает
...
#       - Импортировать ее внутри репозитория для повторного применения синхронно (mutualy connected) или асинхронно (independent) -> Импортировал с асинхронными операциями над ней
...
__myOwnDevelopingVersion__ = 7.94
# todo Связать эту ветку и версию с веткой и версией на github-е и сделать автоинкремент (зависит от объема внесенных изменений), чтобы не давать ее вручную -> Версионирование
#  можно полностью перенести на github в виде релизов (по ветке на каждый релиз). Версия тут - чистая формальность
