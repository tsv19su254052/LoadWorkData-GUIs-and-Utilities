...
В объем данного проекта входит:
 - базы данных (справочные, рабочие и оперативные данные) на внешнем сервере СУБД,
 - прикладное программное обеспечение - утилиты и графические оболочки (в части UX/UI) для работы с ними,
 - работы по администрированию серверов и баз данных на них.
...
Таблицы могут быть в разных базах данных.
Распределение таблиц по базам данных уточняется в процессе разработки и эксплуатации.
Каскадные правила на обновления и удаление данных между ними не применяются.
...
Прикладное программное хранится на файловом сервере и дорабатывается в процессе эксплуатации (в части CI/CD) без уведомления его пользователей.
Права пользователей на доступ к базам данных соответсвуют их учеткам Windows.
Для подписания внесения изменений в справочные данные предусматривается использование ЭЦП на USB-ом токене.
...
Уточнить использование комплектных с **MS SQL** функционала **XPath & XQuery** и спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **SAX**.
...
Все XML-ные поля даем на вход хранимой процедуре через одну XSD-схему (имя корневого тэга соответствует имени элемента XSD-схемы) за один раз.
...
В соответствии с этим относительно корневого тэга вставляем новый тэг и подтэг или вставляем в существующий тэг новый подтэг в соответствии с хронологией.
