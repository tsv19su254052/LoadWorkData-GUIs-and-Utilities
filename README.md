...
(дистрибутив см. на сервере в папке `Q:\Oracle\Oracle SQL Developer\Oracle SQL Developer 21.4.3\sqldeveloper-21.4.3.063.0100-x64`).

Поднять на сервере СУБД ВЭБ-хуки или `Service Broker`, чтобы отказаться от временных задержек на обходах взаимоблокировок и загрузить его максимально.

Сделать графическую формочку для правки свойств альянсов (или делать это внутри `Management Studio`). Добавить ссылку на каждый из `Wikipedia`.
