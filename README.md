...
Версия 8 (в процессе разработки):
 ...
 - если в базе данных нет регистрации, использует функционал версии 7 для временного хранения недозагруженных данных (подробнее см. "Загрузка данных" ниже по тексту), 
...
У авиакомпаний сделать:
 - список аэропортов-хабов XML-ным полем,
 - описание XML-ным полем (разработать структуру разделов и подразделов, возможно взять за образец структуру из `Wikipedia.org`).

Добавить на графической формочке свойтсв авиакомпаний виджеты и ссылки просмотра финансовой и юридической информации из надежной онлайн базы (пока в рабработке).
...
Добавить на графической формочке свойств объектов:
 ...
 - виждеты выбора страны, области (графства, штата, региона), города, района города из надежной онлайн базы на ее API-шках в формате XML
   (как простой пример см. https://htmlweb.ru/geo/api_get_data.php , надо зарегистрироваться, купить и оплачивать API-ключ согласно тарифа https://htmlweb.ru/user/tariffs.php для каждого клиента),
   значение подтэга `wiki` вынести отдельно гиперссылкой, чтобы открывать статью из `Wikipedia.org`,
 - выджеты и ссылки для просмотра свойств вышеперечисленных географических объектов.
...
Предусмотреть возможность вывода истории внесения изменений с указанием подписантов (пока уточняется).
...
Остальные указания и замечания см. в исходниках по тэгам **todo** и **fixme**.
Остальные комментарии см. в исходниках
