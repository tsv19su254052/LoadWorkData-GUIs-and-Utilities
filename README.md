...
Дополнительная часть - утилиты и графические оболочки для работы с ними.
...
#### Решения по архитектуре программного обеспечения (первоначальное, август 2016-го года) `[Рисунок 1]`

 - Сервер СУБД, 
 - Файловый сервер,
 - Терминальный сервер - работают по учеткам Windows без контроллера домена.

Клиенты в локальной подсети или из внешней сети через RDP с термнального сервера используют графические оболочки (в части UX/UI), разрабатываемые в данном проекте.
Выпуск релизов и пакетов не предусматривается, так как программное обеспечение дорабатывается в процессе эксплуатации (в части CI/CD) без уведомления клиентов.
Программное обеспечение хранится на файловом сервере и используется только в оригинале.
