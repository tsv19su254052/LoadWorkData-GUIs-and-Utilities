...
Основная часть данного проекта - базы данных (справочные, рабочие и оперативные данные) на внешнем сервере СУБД.
Дополнительная часть - утилиты и графические оболочки (в части UX/UI) для работы с ними.
...
Пользователи работают в локальной подсети или из внешней сети через термнальный сервер по RDP.
Пользователи используют программное обеспечение, хранящееся на файловом сервере, которое дорабатывается в процессе эксплуатации (в части CI/CD) без уведомления клиентов.
Права клиентов соответсвуют их учеткам Windows. Для подписания внесения изменений в справочные данные предусматривается использование ЭЦП на USB-ом токене.
