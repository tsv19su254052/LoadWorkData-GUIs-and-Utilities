...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов и писать по одному XML-ному полю на арендодателя, лизингодателя, арендатора (владельца) и оператора при их наличии для каждого авиарейса таким образом:
 ...
 - В некоторых XML-ных полях еще одна ветка может быть с аттрибутом пустой авиакомпании. Например, когда на одном авиарейсе владелец и оператор - одна и та же авиакомпания без лизинга или аренды. На другом - одна - владелец, вторая - оператор. На следующемили одна - арендодатель, вторая - арендатор-владелец, третья - оператор. И т. д. в других сочетаниях.
 ...
 - А также убираем предыдущие подветки, если они остаются внутри временного диапазона одной авиакомпании.
