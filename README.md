...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов таким образом:
 - Пишем по одному XML-ному полю на арендодателя, лизингодателя, арендатора (владельца) и оператора для каждого авиарейса.
 - В каждом XML-ном поле у одной авиакомпании может быть только одна ветка с **аттрибутом-идентификатором** (далее по тексту аттрибутом), указывающим на соответствующую запись в базе данных авиакомпаний.
 - В одном XML-ном поле одна ветка может быть с аттрибутом пустой авиакомпании (например, владелец и оператор - одна и та же авиакомпания без лизинга или аренды, или одна - владелец, вторая - оператор, или одна - арендодатель, вторая - арендатор-владелец, третья - оператор и другие сочетания).
 ...
 - В соответствии с этим при необходимости пишем новую ветку и подветку или дописываем в существующую ветку новую подветку.
 ...
 - Группируем подветки в соответствии с хронологией.
 - Убираем предыдущие подветки, если они оказываются внутри временного диапазона одной авиакомпании.
 - Выносим для облегчения отладки запись или обновление каждого XML-ного поля в хранимую процедуру вместе с частью на XPath & XQuery. 
