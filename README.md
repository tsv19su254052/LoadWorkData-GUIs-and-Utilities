...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов и писать в нее по одному XML-ному полю на арендодателя, лизингодателя, арендатора (владельца) и оператора при их наличии для каждого авиарейса таким образом:
 ...
 - В некоторых XML-ных полях может быть еще одна ветка с аттрибутом пустой авиакомпании. Например, когда на одном авиарейсе владелец и оператор - одна и та же авиакомпания без лизинга или аренды. На другом - одна - владелец, вторая - оператор. На следующем одна - арендодатель, вторая - арендатор, третья - оператор. И т. д. в разных сочетаниях.
 ...
 - Считаем, что начало периода использования в следующей авиакомпании совпадает с окончанием периода использования в предыдущей, так как в рабочих данных не указаны периоды бездействия.
