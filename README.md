...
![Таблица с полем типа xml с привязкой к схеме xsd](https://user-images.githubusercontent.com/104857185/167261417-e0820f3d-965f-4124-9af6-e59994e09f46.png)
 ...
 - Считаем, что любая авиакомпания может быть лизингодателем, арендодателем, владельцем или оператором, поэтому все их XML-ные поля в хранимой процедуре пропускаем через одну XSD схему (каждое XML-ное поле через свой элемент XSD схемы в соответствии с именем элемента).
...
То, что в Management Studio называется **коллекцией схем XML**, точнее называть XSD-схемой. Понимает только файл *.xsd. В одной схеме несколько элементов со своими именами. Пространство имен одно на все элементы схемы. Генерируется из XML с помощью Management Studio или с помощью файла *.xslt. Добавляется скриптом._
