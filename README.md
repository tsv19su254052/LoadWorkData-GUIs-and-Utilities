...
_Поправка: В большей части вместо **tk** и **ttk** используется **pyQt**. **Gtk** применяется в Linux-е (библиотека **pyGTK** сейчас на Windows не ставится). Сайт разрабатывается отдельно и в объем данного проекта пока не входит_
