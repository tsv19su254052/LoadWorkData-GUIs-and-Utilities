...
Структура хранилища с базами данных - **Снежинка**. В центре - таблицы с рабочими и оперативными данными, по краям - таблицы со справочными данными, которые ссылаются на таблицы с другими справочными данными. Таблицы с тестовыми рабочими данными вынесены на другой жесткий диск и ссылаются на те же таблицы со справочными данными, что и рабочие. Таблицы могут быть в разных базах данных. Каскадные правила на обновления и удаление данных между ними не применяются. Таблицы со справочными данными вынесены в отдельные базы данных.
...
XML-ное поле хронологии регистрационных номеров летательных аппаратов (англ. _Tail Number_, далее по тексту **регистрации**):
 - В XML-ном поле регистрации летательного аппарата может быть только один тэг с **аттрибутом-строкой** (далее по тексту аттрибутом), обозначающим какую-то одну регистрацию.
 - В одном тэге может быть несколько подтэгов с датами, соответствующими периодам использования (началами временного диапазона) с каждой регистрацией.
 - Считаем, что начало временного диапазона с новой регистрацией совпадает с окончанием временного диапазона с предыдущей регистрацией.
 ...
 - Убираем предыдущий подтэг, если он переходит не в начало временного диапазона с данной регистрацией.
 - Или ничего не пишем, если дата авиарейса попадает в существующий период использования с данной регистрацией.

По одному XML-ному полю на авиакомпанию-лизингодателя, авиакомпанию-арендодателя, авиакомпанию-арендатора (владельца) и авиакомпанию-оператора:
 - Считаем, что любая авиакомпания может быть лизингодателем, арендодателем, владельцем или оператором. 
 - В XML-ном поле авиакомпании на одну авиакомпанию может быть только один тэг с **аттрибутом-идентификатором** (далее по тексту аттрибутом), указывающим на какую-то одну соответствующую запись в базе данных авиакомпаний.
 - Аттрибут может указывать на неизвестного частного владельца (`IATA - пусто`, `ICAO - None`).
 - Аттрибуты в разных XML-ных полях могут указывать на одну и ту же авиакомпанию, например когда авиарейс выполняет авиакомпания-владелец (нет лизинга, нет аренды, нет оператора) или когда авиарейс выполняет авиакомпания-оператор, использующая летательный аппарат авиакомпании-владельца (нет лизинга и нет аренды) или когда авиарейс выполняет авикомпания-оператор на летательном аппарате авиакомпании-арендатора (но без лизинга).
 - В одном тэге может быть несколько подтэгов с датами, соответствующими периодам использования (началами временного диапазона) в каждой авиакомпании.
 - Считаем, что начало временного диапазона в следующей авиакомпании совпадает с окончанием временного диапазона использования в предыдущей.
 - В соответствии с этим вставляем новый тэг и подтэг или вставляем в существующий тэг новый подтэг в соответствии с хронологией.
 - Убираем предыдущий подтэг, если он переходит не в начало временного диапазона в данной авиакомпании.
 - Или ничего не пишем, если дата авиарейса попадает в существующий период использования в данной авиакомпании. 

Для облегчения отладки выносим вставку новой строки или обновление XML-ных полей в существующей строке в хранимую процедуру с помеченными транзакциями вместе с частью на XPath & XQuery. Все XML-ные поля даем на вход хранимой процедуре через одну XSD-схему (каждое XML-ное поле через свой элемент XSD-схемы в соответствии с его именем за один раз). Типы данных указаны в пространстве имен (см. объявление в XSD-схеме).

Примечания:
 - Регистрация с течением времени как правило последовательно меняет 4 ... 5 летательных аппаратов.
 - Летательный аппарат однозначно определяется только сочетанием его заводских номеров (LN, MSN, SN, CN и другие) в зависимости от фирмы-изготовителя летательного аппарата, которое не указано в рабочих и оперативных данных.
 - В версии 3 ... 5 баз данных летательных аппаратов регистрация и авиакомпании не указаны.
 - В версии 6 базы данных летательных аппаратов записаны крайняя регистрация и крайняя авиакомпания.
 - В версии 7 базы данных летательных аппаратов хронология регистраций и авиакомпаний сделана по принципу _медленно меняющейся размерности_ через отдельную таблицу справочных данных (бизнес-ключ - сочетание заводских номеров, суррогатный ключ -  регистрации и авиакомпании по периодам использования соответственно). 
 - В версии 8 базы данных летательных аппаратов хронология регистраций и авиакомпаний сделана на XML-ных полях.
