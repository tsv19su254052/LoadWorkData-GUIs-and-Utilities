...
Доработать базу данных по летательным аппаратам таким образом, чтобы:
 - писать в нее арендодателей, лизингодателей, арендаторов (владельцев) и всех операторов по XML-ному полю на каждую авиакомпанию для каждого авиарейса как аттрибут-идентификатор, соответствующий записи в базе данных авиакомпаний,
 - в каждом XML-ном поле ветка одной авиакомпании была только одна,
 - в одном XML-ном поле одна ветка могла быть с пустой компанией (например, владелец и оператор - одна и та же авиакомпания без лизинга или аренды),
