...
Доработать базу данных по летательным аппаратам таким образом, чтобы:
 - писать в нее арендодателей, лизингодателей, арендаторов (владельцев) и всех операторов по XML-ному полю на каждую авиакомпанию для каждого авиарейса, как внешний ключ, ссылающийся на соответствующую запись в таблице авиакомпаний,
 ...
 - убирать предыдущие подветки, если они оказываются внутри временного диапазона с определенной авиакомпанией,
...
_Считаем, что начало периода использования в следующей авиакомпании совпадает с окончанием периода использования в предыдущей, так как в рабочих данных не указаны периоды бездействия, чтобы избежать сдвоенных записей в подветках.
Недостаток хранимой процедуры - не возвращает в скрипты на Python-е результат своей работы (получилось, не получилось с указанием причины). Недостаток XSD-схемы - тот же и тот, что она пропускает все или не пропускает ничего. Как обычно, просто фиксируем и записываем отказ этого действия в журнал для анализа._ 
