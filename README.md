...
используя комплектный функционал **XPath & XQuery** и спецификацию **SQL/XML** следующим образом:
 ...
 - если в XML-ном поле авиакомпания есть и дата авиарейса более ранняя, чем начало существующего временного диапазона,
   вставляем в существующий тэг с указанным аттрибутом-идентификатором авиакомпании новый подтэг с датой и убираем существующий подтэг (он переходит не в начало этого временного диапазона),
 - если в XML-ном поле авиакомпания есть и дата авиарейса более поздняя, чем начало существующего временного диапазона,
   вставляем в существующий тэг с указанным аттрибутом-идентификатором авиакомпании новый подтэг с датой авиарейса между существующими подтэгами в соответствии с хронологией,
...
В отметке транзакции указываем ___.
