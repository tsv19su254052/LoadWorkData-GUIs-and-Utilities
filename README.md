...
Начало временного диапазона со следующей регистрацией совпадает с окончанием временного диапазона использования с предыдущей. Заполняется только ручным вводом.
...
Заполняется на основании XML-ного поля регистрации при внесении рабочих и оперативных данных.

В соответствии с этим ищем строку летательного аппарата с данной регистрацией в данный период времени
(парсим каждое XML-ное поле регистрации по аттрибуту-строке, делаем это XML-ное поле индексом)
и в каждом XML-ном поле авиакомпании относительно корневого тэга:
 - вставляем новую строку, если строка с данной регистрацией не найдена,
 - вставляем новый тэг и новый подтэг или в существующий тэг вставляем новый подтэг с указанным аттрибутом-идентификатором,
...
Таким образом, XML-ные поля пропускаются через XSD-схему 4 раза:
