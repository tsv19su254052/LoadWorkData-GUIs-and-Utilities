...
Группировка подтэгов с датами авиарейсов (пока в разработке):
 ...
 - вызываем в запросах программно по URL из файлов `*.xq`,
 - пробуем хранимые процедуры с программными вызовами из файлов `*.xq`,
 - вставляем старую XML-ную базу летательного аппарата в XML-ное поле **XML(CONTENT dbo.XSD-схема)** его строки и парсим его **Saxon**-ом (библиотеки `libxml2`, `libxslt`),   
 - во всех его строках переставляем подтэги по хронологии и отбрасываем промежуточные подтэги,
 - определяемся в каких случаях необходимо программно или вручную выполнять группировку.
