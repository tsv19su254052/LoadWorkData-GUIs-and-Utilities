...
Структура хранилища с базами данных - **Снежинка** (в центре - таблицы с рабочими и оперативными данными, с краев - таблицы со справочными данными).
...
#### Работа со справочными данными (а также Замечания, Объемы доработок).
...
#### Загрузка рабочих и оперативных данных (изменяется часть справочных данных)

В рабочих и оперативных данных используется:
 - регистрация,
 - дата авиарейса,
 - строка авиарейса,
...
Загрузка данных обновляет XML-ные поля авиакомпаний **XML(CONTENT dbo.XSD-схема)**,
которые подаются на вход хранимой процедуры через XSD-схему и парсятся как **SAX**,
используя комплектный функционал **XPath & XQuery** и спецификацию **SQL/XML** следующим образом:
 - парсим в таблице летательных аппаратов XML-ное поле регистрации (первичный XML-ный индекс) по тэгу с аттрибутом-строкой с указанной регистрацией,
 ...
 - если строки с регистрацией с подходящим временным периодом нет в базе данных,
   недозагруженные данные на время пишутся во временную таблицу по образцу 7-й версии (см. "История изменений и улучшений в базах данных" в разделе "Версия 8"),
   формируется список соответствия регистраций и их временных диапазонов, которые надо восполнить в базе данных,
 ...
 - подтэги группируем по отдельному алгоритму,
 - если не указана регистрация, то в летательные аппараты изменения не вносятся.
