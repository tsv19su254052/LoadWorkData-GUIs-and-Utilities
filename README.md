...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов для каждого авиарейса следующим образом.

XML-ное поле регистрации летательного аппарата:
 - В каждом XML-ном поле регистрации летательного аппарата может быть только по одной ветке с **аттрибутом-строкой** (далее по тексту аттрибутом), обозначающим регистрацию.
 - В одной ветке может быть одна или несколько подветок с датами, соответствующими периодам использования (началами временного диапазона) с каждой регистрацией.
 ...
 - В соответствии с этим пишем новую ветку и подветку или дописываем в существующую ветку новую подветку в соответствии с хронологией.
 - Убираем предыдущую подветку, если она попадает не в начало временного диапазона с данной регистрацией.
 - Или ничего не пишем, если дата авиарейса попадает в существующий период использования с данной регистрацией.
 - Вставку или обновление XML-ного поля выносим в хранимую процедуру вместе с частью на XPath & XQuery.

По одному XML-ному полю на авиакомпанию-лизингодателя, авиакомпанию-арендодателя, авиакомпанию-арендатора (владельца) и авиакомпанию-оператора при их наличии:
 - В каждом XML-ном поле авиакомпании на одну авиакомпанию может быть только по одной ветке с **аттрибутом-идентификатором** (далее по тексту аттрибутом), указывающим на какую-то одну соответствующую запись в базе данных авиакомпаний.
 ...
 - В одной ветке может быть одна или несколько подветок с датами, соответствующими периодам использования (началами временного диапазона) в каждой авиакомпании.
 ...
 - В соответствии с этим пишем новую ветку и подветку или дописываем в существующую ветку новую подветку в соответствии с хронологией.
 - Убираем предыдущую подветку, если она попадает не в начало временного диапазона в данной авиакомпании.
 - Или ничего не пишем, если дата авиарейса попадает в существующий период использования в данной авиакомпании.
 - Для облегчения отладки выносим вставку или обновление каждого XML-ного поля в хранимую процедуру вместе с частью на XPath & XQuery.
 - Считаем, что любая авиакомпания может быть лизингодателем, арендодателем, владельцем или оператором, поэтому все их XML-ные поля даем на вход хранимой процедуре через одну XSD-схему (каждое XML-ное поле через свой элемент XSD-схемы в соответствии с его именем за один раз).
 
_Примечание: Недостаток хранимой процедуры - не возвращает в скрипты на Python-е результат своей работы (получилось, не получилось с указанием причины). Недостаток XSD-схемы - тот же и тот, что она пропускает все или не пропускает ничего. Делаем как обычно, просто фиксируем и записываем отказ этого действия в журнал для анализа._

###### Структура **XML**-ных полей
...
###### Делаем XSD-схему и привязываем ее на все **XML**-ные поля 
