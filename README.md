...
Предусматривается:
 ...
 - работа клиентов на графических формах (UX/UI) только в локальной подсети по учеткам Windows Server-ов без контроллера домена, с заранее заданной конфигурацией сервера СУБД (имена входа, их права) и с ПО в оригинале с файлового сервера, которое дорабатывается в процессе работы (CI/CD)
