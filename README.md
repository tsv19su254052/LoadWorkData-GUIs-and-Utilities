...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов таким образом, чтобы:
 - писать по одному XML-ному полю на арендодателя, лизингодателя, арендатора (владельца) и оператора для каждого авиарейса,
 - в каждом XML-ном поле ветка с аттрибутом одной авиакомпании была только одна, как аттрибут-идентификатор, соответствующий записи в базе данных авиакомпаний,
 - в одном XML-ном поле одна ветка могла быть с аттрибутом пустой авиакомпании (например, владелец и оператор - одна и та же авиакомпания без лизинга или аренды, или одна - владелец, вторая - оператор, или одна - арендодатель, вторая - арендатор, третья - оператор и другие сочетания),
