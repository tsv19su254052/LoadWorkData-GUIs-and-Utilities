...
То, что в Management Studio называется **коллекцией схем XML**, точнее называть XSD-схемой. Понимает только файл *.xsd. В одной XSD-схеме несколько элементов со своими именами. Пространство имен одно на все элементы XSD-схемы (см. https://www.w3schools.com/xml/schema_simple.asp). Генерируется из XML внутри помощью Management Studio или с помощью файла *.xslt. Добавляется скриптом._
