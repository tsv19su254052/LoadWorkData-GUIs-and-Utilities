...
В объем данного проекта входит:
 ...
 - предоставление способа работы с базами данных из других проектов с помощью API-шек.
...
Оперативные данные загружаются асинхронно на API-шках по ВЭБ-хукам с http://www.flightradar24.com и с первичного оборудования без учета хронологии.

Структура хранилища с базами данных - **Снежинка** (в центре - таблицы с рабочими и оперативными данными (далее по тексту **данными**), с краев - таблицы со справочными данными (далее по тексту **словарями**).
Таблица летательных аппаратов гибридная и содержит:
 - поля, ввод в которые делается только вручную из авторитетных источников,
 - поля, которые заполняются в процессе загрузки рабочих и оперативных данных.
...
Прикладное программное обеспечение хранится на файловом сервере, используется только в оригинале и дорабатывается в процессе эксплуатации (в части CI/CD) без уведомления его пользователей.
...
Версия 3 (устарела):
 - с ошибками в загрузке данных,
 - оставили как образец наработок, источник первичных данных и для анализа.

Версии 4, 5 (теперь не актуальны, сильно загружает процессор сервера СУБД):
...
Версия 6 (текущая):
 ...
 - удачно проиндексированы поля (загрузка ускорилась в 25 ... 35 раз, простои на взаимоблокировках уменьшились),
...
Версия 8 (в процессе разработки):
 ...
 - если в базе данных нет регистрации, использует функционал версии 7 для временного хранения недозагруженных данных, 
...
следующим образом:
 ...
 - выбираем из них строку, у которой временной диапазон данной регистрации соответствует дате авиарейса,
 - если строки с регистрацией с подходящим временным периодом нет в базе данных,
   данные на время пишем во временную таблицу по образцу 7-й версии (см. "История изменений и улучшений в базах данных" в разделе "Версия 8"),
   формируем список соответствия регистраций и их временных диапазонов, которые надо восполнить в базе данных,
   после восполнения переносим данные из временной таблицы к остальным данным,
 - если в XML-ном поле авиакомпании нет,
   вставляем относительно корневого тэга новый тэг с указанным аттрибутом-идентификатором авиакомпании и новый подтэг с датой авиарейса (как новый времменной диапазон),
 - если в XML-ном поле авиакомпания есть,
   вставляем в существующий тэг с указанным аттрибутом-идентификатором авиакомпании новый подтэг с датой в соответствии с хронологией,
 - подтэги группируем по отдельному алгоритму (пока уточняется),
 - если регистрация не указана, то в таблицу летательных аппаратов изменения не вносим,
 - если объекта с указанными данными нет, вставляем новый объект,
 - если маршрута между указанными объектами еще нет, вставляем новый маршрут,
 - если авиарейса с указанными данными нет, вставляем новый авиарейс,
 - если авиарейс к указанными данными уже есть, плюсуем количество этого авиарейса.
...
вставляется по порядку просмотра XML-ных полей. Имя корневого тэга XML-ного поля соответствует имени элемента XSD-схемы.
...
#### Поправкb по терминологии:
 - То, что в Management Studio называется **коллекцией схем XML**, точнее называть XSD-схемой.
 - К каждому XML-ному полю можно привязать свою XSD-схему.
 - То, что SQL-ном скрипте называется **созданием схемы**, правильнее называть привязкой, потому что XSD-схема уже собрана и сохранена файлом типа *.xsd (надо найти способ не вставлять исходник схемы, а выбрать ее файл в диалоге открытия файла, чтобы дать ее на вход скрипту привязки).
