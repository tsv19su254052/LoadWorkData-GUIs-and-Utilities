...
Группировка подтэгов с датами авиарейсов (пока в разработке):
 ...
 - вставляем XML-ную базу по летательному аппарату в XML-ное поле **XML(CONTENT dbo.XSD-схема)** его строки
   и парсим его **Saxon**-ом (библиотеки `libxml2`, `libxslt`),
   в том числе смотрим по остальным строкам, переставляем подтэги по хронологии и отбрасываем промежуточные подтэги,
 - определяемся в каких случаях программно или вручную выполняем группировку.
