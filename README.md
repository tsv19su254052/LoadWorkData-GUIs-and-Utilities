...
История изменений и улучшений в базах данных:
 ...
 - версия 8 - хронология регистраций и авиакомпаний пишется в XML-ных полях, для временного хранения рабочих и оперативных данных, у которых в базе данных еще нет регистрации используется функционал 6-й версии.

###### Делаем **XML**-ные поля

![SSMS Делаем XML-ные поля](https://user-images.githubusercontent.com/104857185/173142801-ff84aa3c-6514-4266-873f-53e4420e909a.png)
...
В соответствии с этим:
 - ищем строки летательных аппаратов и парсим в них XML-ные поля регистрации по тэгу с аттрибутом-строкой с указанной регистрацией (позднее сделаем это XML-ное поле XML-ным индексом),
...
В процессе загрузки рабочих и оперативных данных формируется список регистраций, которые надо восполнить. Недозагруженные данные на время пишутся во временную таблицу. После восполнения они переносятся к остальным данным.
...
Особенность Management Studio такова, что XSD-схему можно создать, привязать и удалить, но нельзя изменить привязанную XSD-схему.
...
Таким образом, XML-ные поля пропускаются через XSD-схему в следующих случаях:
 - при приеме на вход хранимой процедуры,
 - при вставке или обновлении строки,
 - при ручном заполнении XML-ного файла,
 - при ручном вводе XML-ного поля из XML-ного файла.
