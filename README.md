...
Типы данных XML указаны в пространстве имен XSD-схемы (см. `SQLCreateSchemaCollectionXSD.xsd` по пути `..\Запросы на SQL\AirCrafts`).
...
Элементы XSD-схемы генерируются каждый из своего XML-ного поля внутри Management Studio или с помощью XSLT-преобразования из файла `*.xslt`,
вставляются каждый под своим именем в скрипт создания схемы.
