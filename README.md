...
#### Первоначальная идея по архитектуре `[Рисунок 1]`
...
_В большей части вместо **tk** и **ttk** используется **pyQt**. **Gtk** применяется в Linux-е (библиотека **pyGTK** сейчас на Windows не ставится). Сайт разрабатывается отдельно и в объем данного проекта пока не входит_

#### Общая идея по хранению информации в файлах различных типов `[Рисунок 2]`
...
Для обхода попадания на вложенную обработку исключений на клиентах:
 - установить или обновить **двайвер ODBC для MS SQL** (дистрибутив версии 17 и руководство см. в папке "Q:\M$_Windows\SQL_Server\Driver ODBC for SQL Server").
...
#### Делаем схему для всех полей **XML** 
...
_То, что в Management Studio называется коллекцией схем XML, точнее называть XSD-схемой. Генерируется из XML или добавляется скриптом - только *.xsd_
...
Примечания:
 ...
 - Некоторые сайты сделаны без API-шек, поэтому пришлось их парсить
