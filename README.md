...
Модель восстановления баз данных - **Полная**, так как предусматривается одновременная работа с данными множества клиентов и в хранимых процедурах используются помеченные транзакции.
...
следующим образом:
 ...
 - выбираем из них строку, у которой временной диапазон с данной регистрацией соответствует дате авиарейса,
 - если строки с регистрацией с подходящим временным периодом нет в базе данных,
   ...
   после восполнения переносим данные из временной таблицы к остальным данным - это может случиться, если дата авиарейса новее, чем все временные диапазоны с указанной регистрацией,
 ...
 - если в XML-ном поле авиакомпания есть,
   вставляем в существующий тэг с указанным аттрибутом-идентификатором авиакомпании новый подтэг с датой авиарейса в соответствии с хронологией,
...
Запросы на XPath & XQuery правим внутри Management Studio, сохраняем файлами типа `*.xq` и вызываем программно внутри хранимых процедур и SQL-ных скриптов.
...
В XSD-схеме далее определяются несколько элементов, каждый под своим именем (см. https://www.w3schools.com/xml/schema_simple.asp).

Элемент генерируется из XML-ного файла внутри Management Studio или с помощью XSLT-преобразования из файла `*.xslt`и
вставляется в соответствии с порядком просмотра XML-ных полей. Имя корневого тэга XML-ного поля соответствует имени элемента XSD-схемы.
Исходный текст XSD-схемы вставляется в SQL-ный скрипт ее привязки к базе данных (надо найти способ добавить удобства и не вставлять исходник схемы через буфер обмена, а выбирать ее в диалоге открытия файла, чтобы дать на вход скрипту привязки, или хотя бы дать URL до нее `file://P:/...`).
...
#### Поправки по терминологии:
 ...
 - **DTD-схемы** и **XDR-схемы** описаны в документации, но уже не применяются.
...
XML-ные поля пропускаются через XSD-схему:
 ...
 - при ручном заполнении XML-ного файла с привязкой к XSD-схеме в редакторе внутри Management Studio,
...
Найти способ добавить удобства и не вставлять исходник XML-ного файла в строке ввода в диалоге хранимой процедуры внутри Management Studio через буфер обмена, а выбирать его в диалоге открытия файла или хотя бы дать URL до него `file:///P:/...`.
