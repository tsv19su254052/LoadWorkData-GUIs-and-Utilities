...
Доработать базу по летательным аппаратам таким образом, чтобы писать в нее:
 ...
 - операторов
в полях XML(CONTENT XSD-схема), парсить их как DOM по веткам и подветкам и при необходимости дописывать подветки по указанной в аттрибуте дате с например помощью хранимой процедуры.
...
Недостаток XSD-схемы - при обращении к ней внутри скрипта на Python-е она пропускает все или не пропускает ничего, не сообщая причину обратно в скрипт.
...
Дополнительно:
 ...
 - Добавить виджетов на вкладке свойств объекта, а также добавить вкладку ВПП со всеми свойствами (широта, долгота, абс. отметка, длина, ширина и т. д.)
