...
Группировка подтэгов с датами авиарейсов (пока в разработке):
 ...
 - вставляем XML-ную базу летательного аппарата в XML-ное поле **XML(CONTENT dbo.XSD-схема)** его строки и парсим ее **Saxon**-ом (библиотеки `libxml2`, `libxslt`),   
 - в всех его строках переставляем подтэги по хронологии и отбрасываем промежуточные подтэги,
