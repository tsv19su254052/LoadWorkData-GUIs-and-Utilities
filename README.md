...
В начале SQL-ного скрипта создания XSD-схемы объявляются пространства имен для всех элементов
(см. https://www.w3schools.com/xml/schema_simple.asp и https://www.w3.org/TR/xmlschema11-1/) и
...
В XSD-схеме могут быть объявлены несколько элементов, каждый под своим именем.
Элемент генерируется из XML-ного файла внутри Management Studio или с помощью XSLT-преобразования из файла `*.xslt`и
вставляется по порядку просмотра XML-ных полей в SQL-ный скрипт создания XSD-схемы.
Для создания XSD-схемы открыть и запустить собранный SQL-ный скрипт
(см. https://github.com/tsv19su254052/LoadWorkDataAirFlightsDBNew/blob/master/SQL%20%26%20XML%20(XPath%20%26%20XQuery%20XSD%20XDR%20XSLT)/SQLCreateSchemaCollectionXSD.sql).
