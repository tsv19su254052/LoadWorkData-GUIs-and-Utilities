...
(уточняется в процессе разработки и эксплуатации). Есть таблица, в шапке которой есть справочные поля, ввод в которые делается только вручную по нескольким источникам, и поля, которые заполняются в процессе загрузки рабочих и оперативных данных.
