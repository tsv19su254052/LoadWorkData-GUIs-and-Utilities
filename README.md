...
Для облегчения отладки выносим вставку новой строки или обновление XML-ных полей в существующей строке в хранимую процедуру с помеченными транзакциями вместе с частью на XPath & XQuery.
Все XML-ные поля даем на вход хранимой процедуре через одну XSD-схему (каждое XML-ное поле через свой элемент XSD-схемы в соответствии с его именем за один раз).
Типы данных указаны в пространстве имен (см. объявление в XSD-схеме).
...
XML-ное поле регистрации:
 - Может быть только один тэг с **аттрибутом-строкой** (далее по тексту аттрибутом), обозначающим какую-то одну регистрацию.
...
По одному XML-ному полю на авиакомпанию-лизингодателя, авиакомпанию-арендодателя, авиакомпанию-арендатора (владельца) и авиакомпанию-оператора:
 - Может быть только один тэг с **аттрибутом-идентификатором** (далее по тексту аттрибутом), указывающим на какую-то одну соответствующую запись в базе данных авиакомпаний.
 - Аттрибуты в разных XML-ных полях могут указывать на одну и ту же авиакомпанию.
