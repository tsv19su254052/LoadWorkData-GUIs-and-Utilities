...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов и писать в нее XML-ное поле регистрации летательного аппарата и по одному XML-ному полю на авиакомпанию-лизингодателя, авиакомпанию-арендодателя, авиакомпанию-арендатора (владельца) и авиакомпанию-оператора при их наличии для каждого авиарейса следующим образом:
 - В каждом XML-ном поле авиакомпании на одну авиакомпанию может быть только одна ветка с **аттрибутом-идентификатором**, указывающим на соответствующую запись в базе данных авиакомпаний (далее по тексту аттрибутом).
 - В некоторых XML-ных полях может быть ветка с аттрибутом, указывающая на неизвестного частного владельца (`IATA - пусто`, `ICAO - None`).
 - Аттрибуты из разных XML-ных полей авиакомпаний могут указывать на одну и ту же авиакомпанию, например когда авиарейс выполняет авиакомпания-владелец (нет лизинга, нет аренды, нет оператора), когда авиарейс выполняет авиакомпания-оператор, использующая летательный аппарат авиакомпании-владельца (нет лизинга и нет аренды), когда авиарейс выполняет авикомпания-оператор на летательном аппарате авиакомпании-арендатора (но без лизинга).
 ...
 - В соответствии с этим пишем новую ветку и подветку или дописываем в существующую ветку новую подветку в соответствии с хронологией. А также убираем предыдущие подветки, если они попадают не в начало временного диапазона в данной авиакомпании.
