...
Уточнить применение комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы нормально парсить поля **XML(CONTENT dbo.XSD-схема)** в таблицах баз данных.

Доработать базу данных по летательным аппаратам таким образом, чтобы:
 - писать в нее арендодателей, лизингодателей, арендаторов (владельцев) и всех операторов в XML-ных полях на каждом авиарейсе, как внешний ключ, ссылающийся на соответствующую запись в таблице авиакомпаний,
 - парсить их как **DOM** по веткам и подветкам, дописывать новую ветку и подветку или дописывать в существующую ветку новую подветку по указанному аттрибуту с указанной датой в порядке хронологии с помощью хранимой процедуры (чтобы перенести в нее часть на XPath & XQuery). 

Недостаток хранимой процедуры - не возвращает в скрипты на Python-е результат своей работы (получилось, не получилось с указанием причины). Недостаток XSD-схемы - тот же и тот, что она пропускает все или не пропускает ничего.
