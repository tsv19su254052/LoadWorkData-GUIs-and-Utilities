...
В соответствии с этим ищем строку летательного аппарата,
парсим в ней XML-ное поле регистрации относительно корневого тэга по тэгам с указанным аттрибутом-строкой с данной регистрацией в данный период времени
(позднее сделаем это XML-ное поле XML-ным индексом) и в каждом XML-ном поле авиакомпании относительно корневого тэга:
 - вставляем новый тэг и новый подтэг или в существующий тэг вставляем новый подтэг с указанным аттрибутом-идентификатором авиакомпании,
...
В процессе загрузки рабочих и оперативных данных составляется список регистраций и дат, которые не найдены и которые надо внести,
чтобы затем выполнить дозагрузку незагруженным данных.
