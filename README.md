...
Типы данных, применяемые в XML-ных полях, соответствуют объявленным пространствам имен (см. `SQLCreateSchemaCollectionXSD.xsd` по пути `..\SQL & XML (XPath & XQuery XSD XDR XSLT)`).
...
Таким образом, XML-ные поля пропускаются через XSD-схему 3 раза:
