...
Чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **SAX**, используются комплектный с **MS SQL** функционал **XPath & XQuery** и спецификация **SQL/XML**.
...
В отметках транзакций указываются ___.
...
Типы данных, применяемые в XML-ных полях, указаны в пространствах имен XSD-схемы (см. `SQLCreateSchemaCollectionXSD.xsd` по пути `..\Запросы на SQL\AirCrafts`).
