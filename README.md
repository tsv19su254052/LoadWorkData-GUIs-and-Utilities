...
Базы данных (справочные, рабочие и оперативные данные) на внешнем сервере сервере СУБД разрабатываются и администрируются с помощью **Management Studio (SSMS)**.
...
Сервер СУБД работает по учеткам Windows Server-а без контроллера домена.
Клиенты в локальной подсети или из внешней сети через RDP используют графические оболочки (в части UX/UI), разрабатываемые в данном проекте.
...
Программное обеспечение используется только в оригинале на файловом сервере.
