...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов таким образом:
 - Пишем по одному XML-ному полю на арендодателя, лизингодателя, арендатора (владельца) и оператора при их наличии для каждого авиарейса.
 ...
 - А также убираем предыдущие подветки, если они оказываются внутри временного диапазона одной авиакомпании.
