...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в таблицах баз данных и применить в базе данных летательных аппаратов таким образом, чтобы:
 - писать в нее арендодателей, лизингодателей, арендаторов (владельцев) и операторов по одному XML-ному полю на каждую авиакомпанию для каждого авиарейса как аттрибут-идентификатор, соответствующий записи в базе данных авиакомпаний,
 - в каждом XML-ном поле ветка одной авиакомпании была только одна,
 - в одном XML-ном поле одна ветка могла быть с пустой компанией (например, владелец и оператор - одна и та же авиакомпания без лизинга или аренды, или одна - владелец, вторая - оператор, или одна - арендодатель, вторая - арендатор, третья - оператор и другие сочетания),
 - в каждой ветке могло быть одна или несколько подветок с датами по количеству периодов использования (начало временного диапазона) в данной авиакомпании,
 - писать при необходимости новую ветку и подветку или дописывать в существующую ветку новую подветку,
 - начало периода использования в следующей авиакомпании совпадало с окончанием периода использования в предыдущей, так как в рабочих данных не указаны периоды бездействия,
 - группировать подветки в соответствии с хронологией,
 - убирать предыдущие подветки, если они оказываются внутри временного диапазона одной авиакомпании,
