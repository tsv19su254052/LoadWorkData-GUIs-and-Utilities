...
Уточнить использование комплектного с **MS SQL** функционала **XPath & XQuery** и комплектной спецификации **SQL/XML**, чтобы парсить поля **XML(CONTENT dbo.XSD-схема)** как **DOM** в базе данных летательных аппаратов и писать в нее XML-ное поле регистрации летательного аппарата и по одному XML-ному полю на лизингодателя, арендодателя, арендатора (владельца) и оператора при их наличии для каждого авиарейса следующим образом:
 ...
 - В некоторых XML-ных полях может быть еще одна ветка с аттрибутом пустой авиакомпании (неизвестный частный владелец: IATA - пусто, ICAO - None), сведения о которой могут быть уточнены и дополнены позднее.
 - В разных XML-ных полях авиакомпаний аттрибут ветки может ссылаться на одну и ту же авиакомпанию, например когда на одном авиарейсе только владелец (нет лизинга, нет аренды, нет оператора), на втором авиарейсе одна - владелец, другая - оператор (нет лизинга и нет аренды), на третьем авиарейсе одна - арендодатель, вторая - арендатор, третья - оператор (нет лизинга).
