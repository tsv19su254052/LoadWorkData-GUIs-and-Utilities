...
Улучшения в базах данных:
 ...
 - версия 7 - у летательных аппаратов по принципу _медленно меняющейся размерности_
   сделана хронология регистраций (бизнес-ключ - сочетание заводских номеров, суррогатный ключ -  регистрация по периодам использования соответственно), 
...
К каждому XML-ному полю привязывается своя XSD-схема._
...
используя комплектный с **MS SQL** функционал **XPath & XQuery** и спецификацию **SQL/XML**.
...
В одной XSD-схеме может быть несколько элементов со своими именами.
XSD-схем может быть несколько.
...
Пространства имен для всех элементов объявляются в начале скрипта (см. https://www.w3schools.com/xml/schema_simple.asp и https://www.w3.org/TR/xmlschema11-1/).
Типы данных, применяемые в XML-ных полях, соответствуют объявленным пространствам имен (см. `SQLCreateSchemaCollectionXSD.xsd` по пути `..\Запросы на SQL\AirCrafts`).
...
Таким образом, XML-ные поля пропускаются через XSD-схему 2 раза:
